...
async function login(req, res, next) {
    ...
    dbPool.query('select password_hash, user_id, twofa_secret, privilege_level from users where username = $1', [username], async (err, result) => {
        if (err) {
            next(err);
        } else {
            ...
            if (valid) {
                ...
                res.status(200).json({
                    token: jwt.sign({ username, user_id, twofa_enabled: twoFactorRequired, twofa_passed: !twoFactorRequired, privilege_level }, jwtSecret)
                });
            } else {
                res.status(401).json({ error: 2, message: 'Invalid username or password'});
            }
